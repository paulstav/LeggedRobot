...
/*
	...
	Front Right Leg ROS node that reads encoder position in counts from FR_encoder_feedback topic and based on the desired position read from the FR_Position topic, runs a PID controller to calculate the PWM command. The calculated command is published to the FR_pwm_feedback topic.
...
int main(int argc, char **argv)
{
  ...
  // Initialize ROS node
  ros::init(argc, argv, "FR_motor_pid");
  ros::NodeHandle n;
  // Initialize the publisher for PWM data post
  ros::Publisher motor_pid_pub = n.advertise<legged_robot::PWM>("FR_pwm_feedback", 1000);
  // Initialize the subscribers for Encoder data reception and desired position reception
  ros::Subscriber motor_controller_sub = n.subscribe("FR_encoder_feedback", 1000, encoderCallback);
  ros::Subscriber motor_position_sub = n.subscribe("FR_Position", 1000, positionCallback);
  ros::Rate loop_rate(12000);
  
  ROS_INFO("Staring communication with FR Motor Controller node.");
  while (ros::ok())
  {
