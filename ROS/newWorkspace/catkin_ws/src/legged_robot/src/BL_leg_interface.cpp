...
/*
	...
	Back Left Leg Interface ROS node that reads encoders positions in counts from DM35820 card and post it to BL_encoders_feedback topic. It also reads the PWM command from the BL_command_dispatch topic and set the corresponding output to the DM35820 card.
...
int main(int argc, char **argv)
{
  ...
  unsigned long int minor_number = 2;
  ...
  // Initialize ROS node
  ros::init(argc, argv, "BL_leg_interface");
  ros::NodeHandle n;
  // Initialize the publisher for encoder data post
  ros::Publisher motor_interface_pub = n.advertise<legged_robot::LegEncoders>("BL_encoders_feedback", 1000);
  // Initialize the subscribers for PWM command data reception
  ros::Subscriber motor_command_sub = n.subscribe("BL_command_dispatch", 1000, commandCallback);
  ...
  ROS_INFO("Staring communication with BL leg interface node.");
