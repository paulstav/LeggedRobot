...
/*
	...
	ROS node that reads the value of the desired position of the front left (FR) in encoder counts from standard input
	and published the received position in the FR_Position topic.
...
int main(int argc, char **argv)
{
  ...
  // Initialize ROS node
  ros::init(argc, argv, "FR_Read_Position");
  ros::NodeHandle n;
  // Publish for desired position message
  ros::Publisher read_position_pub = n.advertise<legged_robot::Position>("FR_Position", 1000);
