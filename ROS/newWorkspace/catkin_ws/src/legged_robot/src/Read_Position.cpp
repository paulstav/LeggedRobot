...
int main(int argc, char **argv)
{
  ...
  ros::Publisher FL_read_position_pub = n.advertise<legged_robot::Position>("FL_Position", 1000);
  ros::Publisher FR_read_position_pub = n.advertise<legged_robot::Position>("FR_Position", 1000);
  ros::Publisher BL_read_position_pub = n.advertise<legged_robot::Position>("BL_Position", 1000);
  ros::Publisher BR_read_position_pub = n.advertise<legged_robot::Position>("BR_Position", 1000);
  ...
  while (ros::ok())
  {
	...
	else
	{
		...
		if( (rpos !=0) || (rpos == 0 && inputS == "0") )
		{
			...
			FL_read_position_pub.publish(position_msg);
			FR_read_position_pub.publish(position_msg);
			BL_read_position_pub.publish(position_msg);
			BR_read_position_pub.publish(position_msg);
		}
	}
