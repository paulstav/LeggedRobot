...
/*
	...
	Back Left Leg ROS node that reads encoder position in counts via UDP from the Tiva board, publishes the received data to the BL_encoder_feedback topic and listens for published data on the BL_pwm_feedback topic. Once a PWM command message is received it transmits the corresponding command to the Tiva board via UDP.
...
int main(int argc, char **argv)
{
  ...
  // Initialize ROS node
  ros::init(argc, argv, "BL_motor_interface");
  ros::NodeHandle n;
  // Initialize the publisher for Encoder data post
  ros::Publisher motor_interface_pub = n.advertise<legged_robot::Encoder>("BL_encoder_feedback", 1000);
  // Initialize the subscriber for PWM data reception
  ros::Subscriber motor_pid_sub = n.subscribe("BL_pwm_feedback", 1000, pwmCallback);
  ...
  ROS_INFO("Staring communication with BL Tiva board.");
  while (ros::ok())
  {
