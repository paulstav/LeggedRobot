...
/*
	...
	Front Left Leg Interface ROS node that reads encoders positions in counts from DM35820 card and post it to FL_encoders_feedback topic. It also reads the PWM command from the FL_command_dispatch topic and set the corresponding output to the DM35820 card.
...
int main(int argc, char **argv)
{
  ...
  // Initialize ROS node
  ros::init(argc, argv, "FL_leg_interface");
  ros::NodeHandle n;
  // Initialize the publisher for encoder data post
  ros::Publisher motor_interface_pub = n.advertise<legged_robot::LegEncoders>("FL_encoders_feedback", 1000);
