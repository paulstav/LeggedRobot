...
/*
	...
	Back Right Leg ROS node that reads encoder position in counts from BR_encoder_feedback topic and based on the desired position read from the BR_Position topic, runs a PID controller to calculate the PWM command. The calculated command is published to the BR_pwm_feedback topic.
